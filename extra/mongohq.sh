...
cat | tee "$BUILD_DIR"/.profile.d/mongo.sh <<EOF
  ...
  export MONGO_URL=\${MONGO_URL:-`ruby -e "require 'json'; puts JSON.parse(ENV['VCAP_SERVICES']).find { |key,value| value[0]['credentials'][uri].start_with? 'mongodb' }[1][0]['credentials']['uri']"`}
EOF
